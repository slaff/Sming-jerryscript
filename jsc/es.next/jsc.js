...
var JSC = {
 ...
 writeToFile: function(outputPath, outputArray) {
  ...
  if (!ENVIRONMENT_IS_NODE) {
   throw new Error("Writing to file is only available when running with Node.js");
  }
  var fs = require("fs");
 ...
 compileBuffer: function(inBuffer, inBufferSize, isForGlobal, isStrict, outputPath) {
  ...
  var emscripten_snapshot_compiler_compile = Module.cwrap("emscripten_snapshot_compiler_compile", "number", [ inBufferType, "number", "number", "number", "number", "number" ]);
  ...
  var rv = Module.HEAPU8.subarray(snapshotOutputBuffer, snapshotOutputBuffer + bytesWritten);
 ...
 compileString: function(sourceString, isForGlobal, isStrict, outputPath) {
  ...
  var rv = this.compileBuffer(sourceUTF8Buffer, sourceBufferSize - 1, isForGlobal, isStrict, outputPath);
  Module._free(sourceUTF8Buffer);
  return rv;
 },
 compileFile: function(inputPath, isForGlobal, isStrict, outputPath) {
  if (!ENVIRONMENT_IS_NODE) {
   throw new Error("compileFile() is only available when running with Node.js");
  }
  var fs = require("fs");
  var sourceInputBuffer = fs.readFileSync(inputPath);
  return this.compileBuffer(sourceInputBuffer, sourceInputBuffer.length, isForGlobal, isStrict, outputPath);
 }
};
...
function _clock_time_get(clk_id, precision_low, precision_high, ptime) {
 ...
 SAFE_HEAP_STORE(ptime | 0, nsec >>> 0 | 0, 4);
 SAFE_HEAP_STORE(ptime + 4 | 0, nsec / Math.pow(2, 32) >>> 0 | 0, 4);
 return 0;
}
...
var _emscripten_snapshot_compiler_get_version = Module["_emscripten_snapshot_compiler_get_version"] = function() {
 return (_emscripten_snapshot_compiler_get_version = Module["_emscripten_snapshot_compiler_get_version"] = Module["asm"]["emscripten_snapshot_compiler_get_version"]).apply(null, arguments);
};

var _emscripten_snapshot_compiler_compile = Module["_emscripten_snapshot_compiler_compile"] = function() {
 return (_emscripten_snapshot_compiler_compile = Module["_emscripten_snapshot_compiler_compile"] = Module["asm"]["emscripten_snapshot_compiler_compile"]).apply(null, arguments);
};
...
Module["onRuntimeInitialized"] = function() {
 if (ENVIRONMENT_IS_NODE && require.main === module) {
  ...
  JSC.compileFile(options.inputPath, options.isForGlobal, options.isStrict, options.outputPath);
  process.exit(0);
 }
};
